...
export default function PostRunActions({ result }: PostRunActionsProps) {
          ...
          <div>
            ...
            <div className="flex">
              <input
                type="text"
                readOnly
                value={`https://coreframe.ai/mcp/run?tool=calculate_time_value&monthly_income=${result.hourlyRate * 40 * 4}&working_hours=40`}
...
async function calculateTimeValue(monthlyIncome, workingHours) {
  const response = await fetch('https://api.coreframe.ai/mcp/calculate_time_value', {
    method: 'POST',
